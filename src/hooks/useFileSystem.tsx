...
export const useFileSystem = (props: UseFileHandlingHookProps) => {
    ...
    const processWatchedDirectories = useCallback(async () => {
        ...
        for (const [
            directoryPath,
            directoryHandle,
        ] of Array.from(watchedDirectoriesRef.current.entries())) {
        ...
        for (const filePath of Array.from(temporaryHandles.keys())) {
    ...
    return {
        ...
        files,
        directoryHandle: Array.from(watchedDirectoriesRef.current.values())[0],
        isProcessing,
